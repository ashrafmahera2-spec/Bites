...
export default function AdminKitchen() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      toast.error('خطأ في تحميل طلبات المطبخ');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6" dir="rtl">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            ...
            <select
              className="pr-10 pl-4 py-2 rounded-xl border border-gray-200 focus:ring-2 focus:ring-orange-500 outline-none appearance-none bg-white text-sm disabled:bg-gray-50 disabled:text-gray-500"
              value={selectedBranchId}
              onChange={(e) => setSelectedBranchId(e.target.value === 'all' ? 'all' : Number(e.target.value))}
              disabled={user?.role !== 'admin'}
            >
              {user?.role === 'admin' && <option value="all">جميع الفروع</option>}
              {branches.map(b => (
      ...
      {loading && orders.length === 0 ? (
        ...
        <div className="bg-white p-12 rounded-3xl shadow-sm border border-gray-100 text-center">
          ...
          <h3 className="text-xl font-bold text-gray-900">لا توجد طلبات حالية</h3>
          <p className="text-gray-500 mt-2">المطبخ فارغ، استمتع بالهدوء!</p>
        </div>
      ) : (
            ...
            {orders.map((order) => (
              ...
              >
                ...
                <div className={`p-4 flex justify-between items-center ${
                  ...
                  <div>
                    <span className="text-xs font-bold text-gray-500 block mb-1">رقم الطلب: #{order.id}</span>
                    <h3 className="font-bold text-gray-900">{order.customerName || 'عميل محلي'}</h3>
                  </div>
                ...
                <div className="p-4 border-t border-gray-50 bg-gray-50/50 space-y-3">
                  <div className="flex items-center justify-between text-xs text-gray-500">
                    <div className="flex items-center gap-1">
                      <Building2 size={14} />
                      <span>{branches.find(b => b.id === order.branchId)?.name || 'غير محدد'}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <Clock size={14} />
                    ...
                    <div className="flex items-center gap-1">
                      <AlertCircle size={14} />
                      <span>{order.type === 'delivery' ? 'توصيل' : 'تيك أوي'}</span>
                    </div>
                  </div>
